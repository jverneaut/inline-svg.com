<script>
  ...
  export const TAGS = ['Chevron', 'Arrow', 'Left', 'Direction'];
  ...
  $: xOffset = width * ratio;
  $: yOffset = (ratio * height) / 2;
</script>

<svelte:options accessors={true} />
<svg fill={color} {width} {height} xmlns="http://www.w3.org/2000/svg">
  <path
    d={`M${width} ${yOffset} L${width} 0 L0 ${height / 2} L${width} ${height} L${width} ${height - yOffset} L${xOffset} ${height / 2}`} />
</svg>
